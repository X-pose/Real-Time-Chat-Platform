...
exports.generateNewAccessToken = (req,res) => {
    ...
    const token = req.headers.authorization && req.headers.authorization.split(' ')[1]
     ...
     //Checks for missing tokens
     if (!token) {
        res.status(HttpStatus.UNAUTHORIZED).json({ body: sysWarn.MISSING_JWT_TOKEN}) 
    }
    ...
    try {
        ...
        const accessToken = jwt.sign({ userId: userId, userType: 'CLIENT' }, process.env.JWT_SECRET, { expiresIn: '1h' })

        res.status(HttpStatus.OK).json({accessToken: accessToken }) 

    } catch (error) {
        //If error occured during decode phase, responds with Invalid token message.
        res.status (HttpStatus.UNAUTHORIZED).json({ body: sysWarn.INVALID_JWT_TOKEN})
    }

}

//Generates access token 
exports.getAccessToken = (userId,userType) => {

    const accessToken = jwt.sign({ userId: userId, userType: userType }, process.env.JWT_SECRET, { expiresIn: '1h' })
...
exports.getRefreshToken = (userId) => {
    ...
    const refreshToken = jwt.sign({ userId: userId}, process.env.JWT_SECRET, { expiresIn: '90d' })
